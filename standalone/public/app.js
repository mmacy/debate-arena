// Page logic for the standalone Debate Arena.
//
// Transport shim: `copilot.<method>(...args)` is a POST to /api/rpc, and the
// server's streaming pushes arrive as Server-Sent Events that we dispatch to
// `window.debateUI.<fn>(...)`. That keeps the exact contract the debate engine
// expects, so the orchestration code is identical to the extension's.

// ---------------------------------------------------------------------------
// RPC: window.copilot.<method>(...args) -> POST /api/rpc
// ---------------------------------------------------------------------------
const copilot = new Proxy(
  {},
  {
...
// ---------------------------------------------------------------------------
// SSE: server pushes { fn, args } -> window.debateUI[fn](...args)
// ---------------------------------------------------------------------------
function connectEvents() {
  ...
  es.addEventListener("push", (ev) => {
    ...
    const fn = window.debateUI && window.debateUI[msg.fn];
...
function setConnected(ok) {
  if (ok) {
    everConnected = true;
    if (els.status.classList.contains("error") && els.statusText.textContent === "Disconnected") {
      setStatus("idle", "Idle");
    }
  } else if (everConnected) {
    // Only surface a drop once we've been live — EventSource auto-reconnects.
    if (!isRunning) setStatus("error", "Disconnected");
  }
}
...
const els = {
  ...
  setupMsg: $("setupMsg"),
  status: $("statusPill"),
  statusText: document.querySelector("#statusPill .status-text"),
  stage: $("stage"),
  empty: $("empty"),
  proWin: $("proWin"),
  conWin: $("conWin"),
  drawWin: $("drawWin"),
  proCorner: document.querySelector(".corner.pro"),
  conCorner: document.querySelector(".corner.con"),
  saveBar: $("saveBar"),
  filename: $("filename"),
  save: $("saveBtn"),
  copy: $("copyBtn"),
...
let isRunning = false;
const messages = new Map(); // id -> { raw, contentEl }
...
function hideWinPills() {
  ...
  els.proCorner.classList.remove("won");
  els.conCorner.classList.remove("won");
}
...
async function loadModels() {
  ...
  try {
    ...
    setSetupMsg(`${models.length} model${models.length === 1 ? "" : "s"} ready.`);
...
// ---------------------------------------------------------------------------
function nearBottom() {
  return els.stage.scrollHeight - els.stage.scrollTop - els.stage.clientHeight < 140;
}
function scrollDown(force) {
  if (force || nearBottom()) els.stage.scrollTop = els.stage.scrollHeight;
}
function clearStage() {
  els.stage.querySelectorAll(".msg, .phase-divider, .victory").forEach((n) => n.remove());
  messages.clear();
...
window.debateUI = {
  debateStarted() {
    ...
    setStatus("running", "In session");
  },

  phase(label) {
    const div = document.createElement("div");
    div.className = "phase-divider";
    div.innerHTML = `<span class="pip">◆</span> ${escapeHtml(label)} <span class="pip">◆</span>`;
  ...
  debateFinished(info) {
    setStatus("done", "Verdict in");
    hideWinPills();
    const pill = info.winner === "pro" ? els.proWin : info.winner === "con" ? els.conWin : els.drawWin;
    if (info.winnerLabel) pill.title = info.winnerLabel;
    pill.hidden = false;
    if (info.winner === "pro") els.proCorner.classList.add("won");
    if (info.winner === "con") els.conCorner.classList.add("won");
    appendVictory(info);
    showSaveBar();
    scrollDown(true);
    finishRun();
...
function appendVictory(info) {
  const banner = document.createElement("div");
  banner.className = `victory ${info.winner}`;
  const title =
    info.winner === "pro" ? "Pro takes the match" :
    info.winner === "con" ? "Con takes the match" :
    "The match is a draw";
  const sub = info.winner === "draw" ? "Honors even" : escapeHtml(info.winnerLabel || "");
  banner.innerHTML = `
    <div class="victory-kicker">The verdict</div>
...
function slug(s) {
  return s.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-+|-+$/g, "").slice(0, 50) || "debate";
}
...
async function start() {
  ...
  if (!config.topic) {
    setSetupMsg("Enter a resolution to debate.", true);
    els.topic.focus();
    return;
  }
...
async function writeClipboard(text) {
  ...
  // Fallback for the rare non-secure context.
...
async function copy() {
  ...
  try {
    const md = await copilot.copyDebate(lastResult);
    await writeClipboard(md);
    els.saveResult.className = "save-result ok";
    els.saveResult.textContent = `Copied to clipboard ✓ (${md.length.toLocaleString()} chars)`;
...
els.topic.addEventListener("keydown", (e) => {
  if ((e.metaKey || e.ctrlKey) && e.key === "Enter" && !isRunning) start();
});
